    ...
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-20 bg-black/90 backdrop-blur-md border-b border-white/5 shadow-2xl">
      <div className="max-w-7xl mx-auto h-full px-8 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          <div className="flex gap-3 xl:gap-6 text-[10px] uppercase tracking-[0.2em] font-bold text-white/60">
            {navLinks.map((link) => (
              <button
                key={link.id}
          ...
          <button
            disabled
            className="px-6 py-2.5 bg-gray-500 text-black text-[10px] font-black uppercase tracking-widest rounded-full cursor-not-allowed opacity-50"
          >
            Iscrizioni Chiuse
          </button>
        </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <button
                disabled
                className="mt-4 px-6 py-3 rounded-full text-center text-black font-bold bg-gray-500 cursor-not-allowed opacity-50"
              >
                Iscrizioni Chiuse
              </button>
            </div>
