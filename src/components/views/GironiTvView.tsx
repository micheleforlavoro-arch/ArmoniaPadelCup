...
const GironiTvView = () => {
  ...
  return (
    ...
    >
      ...
      {/* Griglia Gironi - Flessibile con altezza calcolata per evitare sovrapposizioni */}
      <div className="w-full flex-1 grid grid-cols-3 grid-rows-2 gap-4 px-6 my-4 min-h-0">
        {gironiList.map(gName => {
          ...
          return (
            <div key={gName} className="bg-white/[0.03] border border-white/10 rounded-2xl p-4 flex flex-col justify-between backdrop-blur-sm min-h-0 overflow-hidden shadow-2xl">
              {/* Intestazione Girone */}
              <div className="flex justify-between items-center mb-1 border-b border-white/15 pb-1">
                <span className="font-black text-xl text-[#A5D8FF] italic tracking-wide">GIRONE {gName}</span>
              </div>
              
              <div className="flex-1 flex flex-col justify-between min-h-0 gap-2 mt-1">
                {/* Squadre */}
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {groupTeams.slice(0, 4).map((team: string, idx: number) => (
                      <div key={`tv-team-${idx}`} className={`text-[1.2vh] uppercase font-black px-2 py-0.5 rounded border border-white/5 bg-white/5 truncate ${team ? 'text-white' : 'text-white/20 italic'}`}>
                        {team || `Team ${idx + 1}`}
                      </div>
                    ))}
                  </div>
                ...
                {/* Partite */}
                <div className="flex-1 flex flex-col justify-between min-h-0 border-t border-white/10 pt-1.5">
                      ...
                      return (
                        <div key={`tv-match-${idx}`} className="flex flex-col justify-center py-0.5 border-b border-white/5 last:border-0 min-h-0">
                          {hasContent ? (
                            <>
                              ...
                              <div className="flex justify-between items-center w-full min-h-0">
                                <div className="flex-1 flex items-center gap-1.5 min-w-0">
                                  <span className="text-[#A5D8FF]/30 font-black text-[1.1vh]">{idx + 1}.</span>
                                  <span className="font-black text-[1.25vh] uppercase tracking-wide truncate text-[#A5D8FF]">
                                    {home || '-'} <span className="text-white/30 text-[0.9vh] font-normal mx-0.5">vs</span> {away || '-'}
                                  </span>
                                </div>
                                {score && (
                                  <span className="bg-[#A5D8FF] text-black font-black text-[1.1vh] px-1.5 py-0.25 rounded-md min-w-[2.2ch] text-center shadow-lg ml-2">
                                    {score}
                                  </span>
                                )}
                              </div>
                              {/* Data, Ora, Campo */}
                              {(dateTime || court) && (
                                <div className="flex items-center gap-2 text-[0.9vh] font-bold uppercase tracking-wider text-white/40 mt-0.5 ml-4">
                            ...
                            <div className="flex items-center gap-1.5 text-white/10 italic text-[1.1vh]">
                              <span>{idx + 1}.</span>
