...
const TabelloneTvView = () => {
  ...
  return (
    ...
    >
      ...
      <div className="z-10 w-full h-full p-8 flex flex-col justify-between">
        {/* Intestazione */}
        ...
        {/* Tabellone */}
        <div className="flex-1 flex justify-center items-stretch w-full gap-4 px-4 pb-2 min-h-0">
            ...
            {[0, 1, 2, 3, 4, 5, 6, 7].map(i => (
              <div key={`ottavi-${i}`} className="flex-1 min-h-0 flex flex-col justify-center my-0.5">
                <div className="px-3 py-1 rounded-xl border border-white/15 bg-black/75 backdrop-blur-sm flex flex-col justify-center h-full max-h-[8vh] shadow-lg">
                  <div className="h-1/2 flex items-center px-1 text-[1.3vh] font-bold uppercase truncate border-b border-white/10 text-white">
                    {renderTeam(tournamentState.bracket?.ottavi?.[i * 2])}
                  </div>
                  <div className="h-1/2 flex items-center px-1 text-[1.3vh] font-bold uppercase truncate text-white">
          ...
          {/* Quarti */}
          <div className="flex flex-col justify-around flex-1 h-full py-4">
            ...
            {[0, 1, 2, 3].map(i => (
              <div key={`quarti-${i}`} className="flex-1 min-h-0 flex flex-col justify-center my-1 relative">
                ...
                <div className="px-3 py-1.5 rounded-xl border-l-4 border-white/30 bg-black/75 backdrop-blur-sm flex flex-col justify-center h-full max-h-[12vh] shadow-lg">
                  <div className="h-1/2 flex items-center px-1.5 text-[1.5vh] font-black uppercase truncate border-b border-white/10 text-white">
                    {renderTeam(tournamentState.bracket?.quarterFinals?.[i * 2])}
                  </div>
                  <div className="h-1/2 flex items-center px-1.5 text-[1.5vh] font-black uppercase truncate text-white">
          ...
          {/* Semifinali */}
          <div className="flex flex-col justify-around flex-[1.1] h-full py-10">
          ...
          {/* Finale */}
          <div className="flex flex-col justify-around flex-[1.1] h-full py-20">
          ...
          {/* Finalissima */}
          <div className="flex flex-col justify-center flex-[1.3] h-full py-8">
             ...
             <div className="relative p-6 rounded-[28px] border-2 border-yellow-500/30 bg-black/85 backdrop-blur-sm bg-gradient-to-b from-yellow-500/10 to-transparent shadow-[0_0_40px_rgba(250,176,5,0.15)] overflow-hidden">
                ...
                <div className="flex flex-col h-[16vh] justify-center">
                  <div className={`h-1/2 flex items-center justify-center px-2 text-[2.5vh] font-black tracking-tight uppercase italic truncate border-b border-white/10 ${tournamentState.bracket?.finalissima?.[0] ? 'text-yellow-500' : 'text-white/20'}`}>
                    {renderTeam(tournamentState.bracket?.finalissima?.[0], true)}
                  </div>
                  <div className={`h-1/2 flex items-center justify-center px-2 text-[2.5vh] font-black tracking-tight uppercase italic truncate ${tournamentState.bracket?.finalissima?.[1] ? 'text-yellow-500' : 'text-white/20'}`}>
                ...
                {tournamentState.bracket?.winner && (
                  <motion.div 
                    ...
                    className="mt-3 pt-3 border-t border-yellow-500/30 text-center"
                  >
                    <span className="text-[1.1vh] font-black uppercase tracking-[0.3em] text-yellow-500 block mb-1">Campione</span>
                    <span className="font-black text-[3vh] text-yellow-400 italic uppercase drop-shadow-[0_0_15px_rgba(250,176,5,0.75)] truncate block">
                      🏆 {tournamentState.bracket.winner} 🏆
                    </span>
                  </motion.div>
                )}
